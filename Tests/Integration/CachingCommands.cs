...
try
{
    ...
    try
    {
        ...
        var cachedCmd = Run("bash", "-c", $"echo 'execution' >> {testFile}").Cached();
...
try
{
    ...
    var cachedCmd = Run("date", "+%N").Cached(); // Nanoseconds for precision
    
    var result1 = await cachedCmd.GetStringAsync();
    await Task.Delay(10); // Small delay to ensure time would change
    var result2 = await cachedCmd.GetStringAsync();
    var result3 = await cachedCmd.GetStringAsync();
...
try
{
    var cachedCmd = Run("echo", "line1\nline2\nline3").Cached();
    
    var lines1 = await cachedCmd.GetLinesAsync();
    var lines2 = await cachedCmd.GetLinesAsync();
    var lines3 = await cachedCmd.GetLinesAsync();
...
try
{
    var nonCachedCmd = Run("date", "+%N"); // No .Cached() call
    
    var result1 = await nonCachedCmd.GetStringAsync();
    await Task.Delay(10); // Small delay
    var result2 = await nonCachedCmd.GetStringAsync();
...
{
    var testData = "apple\nbanana\napricot\navocado";
    var cachedPipeline = Run("echo", testData)
        .Pipe("grep", "a")
        .Pipe("sort")
        .Cached();
    
    var result1 = await cachedPipeline.GetLinesAsync();
    var result2 = await cachedPipeline.GetLinesAsync();
...
try
{
    ...
    var cachedFind = Run("echo", "file1.cs\nfile2.txt\nfile3.cs\nfile4.md").Cached();
    
    // Different filters on the same cached base
    var csFiles = await cachedFind.Pipe("grep", ".cs").GetLinesAsync();
    var txtFiles = await cachedFind.Pipe("grep", ".txt").GetLinesAsync();
    var allFiles = await cachedFind.GetLinesAsync();
...
try
{
    var cmd1 = Run("echo", Guid.NewGuid().ToString()).Cached();
    var cmd2 = Run("echo", Guid.NewGuid().ToString()).Cached();
    
    var result1 = await cmd1.GetStringAsync();
    var result2 = await cmd2.GetStringAsync();
...
try
{
    var baseCmd = Run("echo", "test_data");
    
    // First call cached
    var cachedResult1 = await baseCmd.Cached().GetStringAsync();
    var cachedResult2 = await baseCmd.Cached().GetStringAsync();
    
    // These should be equal (both cached)
    if (cachedResult1 == cachedResult2)
    {
        // Now call without caching - should still work
        var freshResult = await baseCmd.GetStringAsync();
...
try
{
    ...
    var cachedCmd = Run("echo", "cancellation_test").Cached();
    
    var result1 = await cachedCmd.GetStringAsync(cts.Token);
    var result2 = await cachedCmd.GetStringAsync(cts.Token);
...
try
{
    var cachedCmd = Run("nonexistentcommand123").Cached();
    
    var result1 = await cachedCmd.GetStringAsync();
    var result2 = await cachedCmd.GetStringAsync();
    var lines1 = await cachedCmd.GetLinesAsync();
    
    if (result1 == string.Empty && result2 == string.Empty && lines1.Length == 0)
    {
